...
export default function HeroSection({ title, subtitle, ctaText, ctaHref, bgImage, showLogo, children }: HeroSectionProps) {
  ...
  return (
    <section ref={ref} className="relative flex items-center justify-center min-h-[70vh] md:min-h-screen overflow-hidden">
      {bgImage && (
        <motion.div style={{ y }} className="absolute inset-0 scale-[1.03]">
          <Image
            src={bgImage}
            alt=""
            fill
            className="object-cover object-[67%_70%]"
            priority
          />
        </motion.div>
      )}
